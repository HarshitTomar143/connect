...
export const getSettings = asyncHandler(async (req, res) => {
  const user = await User.findById(req.user._id).select(
    "readReceiptsEnabled shareLocation showLastSeen location"
  );
  res.json(user);
});

export const updateSettings = asyncHandler(async (req, res) => {
  const { readReceiptsEnabled, shareLocation, showLastSeen, location } = req.body;
  const updates = {};
  ...
  const user = await User.findByIdAndUpdate(req.user._id, updates, {
    new: true,
    runValidators: true,
    select: "readReceiptsEnabled shareLocation showLastSeen location",
  });
  res.json(user);
});
